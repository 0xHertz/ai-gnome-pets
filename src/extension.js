...
export default class DesktopGnomeletsExtension extends Extension {
    enable() {
        ...
        this._conversationManager = new ConversationManager(this._settings, this._manager);
    ...
    _setupInteraction() {
        if (!this._conversationManager.isEnabled()) return;

        const interactionTimer = () => {
            if (this._conversationManager.isEnabled() && Math.random() < 0.05) {
                this._conversationManager.triggerPetInteraction();
            }
            return GLib.timeout_add(GLib.PRIORITY_DEFAULT, 60000, interactionTimer);
        };
    ...
    _updateIndicator() {
        ...
        if (show) {
            if (!this._indicator) {
                this._indicator = new GnomeletIndicator(this, this._conversationManager);
