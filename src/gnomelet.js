...
export const Gnomelet = GObject.registerClass(
  class Gnomelet extends GObject.Object {
    ...
    _updateDimensions() {
      ...
      this._displayW = Math.floor(
        this._displayH * (this._frameWidth / this._frameHeight),
      );
    ...
    updateJumpPower() {
      ...
      this._jumpReachX =
        WALK_SPEED * 2 * Math.abs(this._jumpVelocity / GRAVITY);
      this._maxJumpHeight =
        (this._jumpVelocity * this._jumpVelocity) / (2 * GRAVITY);
    }
    ...
    _updateInteraction() {
      ...
      if (allowInteraction) {
        if (!this._draggable) {
          ...
          this._dragBeginId = this._draggable.connect(
            "drag-begin",
            this._onDragBegin.bind(this),
          );
          this._dragEndId = this._draggable.connect(
            "drag-end",
            this._onDragEnd.bind(this),
          );
        }
      } else {
        // Keep reactive for tooltip, but remove draggable
        if (this._draggable) {
          this._draggable = null;
        }
      }

      // Always keep actor reactive for tooltip and click interaction
      this.actor.reactive = true;
    ...
    _onDoubleClick() {
      if (this._conversationManager && this._conversationManager.isEnabled()) {
        ...
        if (isCurrentChat) {
          ...
          this._conversationManager.triggerChat(this._petId);
        }
      }
    }

    setConversationManager(convManager, petId) {
      this._conversationManager = convManager;
      this._petId = petId;
      if (convManager) {
        convManager.initPet(petId, this._typeName);
      }
    }

    startChatting(isPairChat = false) {
    ...
    stopChatting() {
      if (this._savedState !== null) {
        ...
        this._vx = this._savedVx || 0;
        this._vy = this._savedVy || 0;
    ...
    updateLoadingBubblePosition() {
      if (this._bubbleContainer && this._loadingInterval) {
        const bubbleWidth = 80;
    ...
    showBubble(message, isOwner = false) {
      this._hideBubble();

      if (this._loadingInterval) {
        GLib.source_remove(this._loadingInterval);
    ...
    _hideBubble() {
      if (this._bubbleTimeout) {
        GLib.source_remove(this._bubbleTimeout);
      ...
      if (this._bubbleContainer) {
        Main.uiGroup.remove_child(this._bubbleContainer);
        this._bubbleContainer.destroy();
        this._bubbleContainer = null;
        this._bubbleActor = null;
      }
    }
    ...
    updateBubblePosition() {
      ...
      if (this._conversationManager) {
        this._conversationManager.updateInputBubblePosition();
      }
    }
    ...
    handleDragOver(source, actor, x, y) {
      ...
      while (
        ...
        now - this._dragHistory[0].time > HISTORY_LIMIT_MS
      ) {
    ...
    update(windows, forceBackground, dockContainer) {
      ...
      let monitors = Main.layoutManager.monitors.filter(
        (m) => feetX >= m.x && feetX < m.x + m.width,
      );
      ...
      if (this._vy >= 0) {
        ...
        for (let win of windows) {
          ...
          let inHorizontalRange =
            feetX >= rect.x && feetX <= rect.x + rect.width;
      ...
      if (landedOnWindow && !landedOnWindow.isDock) {
        ...
        if (!forceBackground) {
          ...
          if (
            dockContainer &&
            this.actor.get_parent() === Main.layoutManager.uiGroup &&
    ...
    _randomizeStartPos() {
      ...
      if (!monitors || monitors.length === 0) {
        this._x = Math.floor(
          Math.random() * (global.stage.width - this._displayW),
        );
        this._y = 0;
      } else {
    ...
    _updateAnimation() {
      ...
      switch (this._state) {
        case State.WALKING:
          ...
          let idx =
            Math.floor(this._animationTimer / speed) % walkFrames.length;
     ...
     */
    setFrame(frameIndex) {
      if (this._frame === frameIndex && this._lastFacing === this.facingRight)
        return;
