...
export const GnomeletManager = GObject.registerClass(
    class GnomeletManager extends GObject.Object {
        ...
        setConversationManager(convManager) {
            ...
            for (let i = 0; i < this._gnomelets.length; i++) {
                this._gnomelets[i].setConversationManager(convManager, `pet_${i}`);
            }
        }
        ...
        _updateCount() {
            ...
            if (count > current) {
                for (let i = 0; i < (count - current); i++) {
                    ...
                    let petId = `pet_${current + i}`;
                    let gnomelet = new Gnomelet(res.type, res.frames, res.w, res.h, this._settings, provider);
                    gnomelet.setConversationManager(this._conversationManager, petId);
                    this._gnomelets.push(gnomelet);
                }
        ...
        _spawnGnomelets(savedState) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                p.setConversationManager(this._conversationManager, petId);
